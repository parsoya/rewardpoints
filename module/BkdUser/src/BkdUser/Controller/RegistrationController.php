...
use BkdUser\Entity\User;
use BkdUser\Options\ModuleOptions;
use BkdUser\Service\UserService as UserCredentialsService;
...
class RegistrationController extends AbstractActionController
{
    ...
    public function indexAction()
    {
        ...
        if($this->getRequest()->isPost()) {
            ...
            if($form->isValid()) {
                ...
                $user->setState($entityManager->find('BkdUser\Entity\State', 1));
                $user->setRole($entityManager->find('BkdUser\Entity\Role', 2));
                $user->setEmailConfirmed(false);
                $user->setLanguage($entityManager->find('BkdUser\Entity\Language', 1));
    		    ...
    		    try {
                    ...
                    $viewModel->setTemplate('bkd-user/registration/registration-success');
                    return $viewModel;
    		    } catch (\Exception $e) {
                    return $this->getServiceLocator()->get('bkduser_error_view')->createErrorView(
        ...
        $viewModel->setTemplate('bkd-user/registration/registration');
        return $viewModel;
    }
        ...
        if($this->getRequest()->isPost()) {
            ...
            if($form->isValid()) {
                ...
                if($identicalValidator->isValid($data->getAnswer())) {
                    ...
                    $viewModel->setTemplate('bkd-user/registration/change-password-success');
                    return $viewModel;
                } else {
    ...
    public function resetPasswordAction()
    {
        ...
        if($this->getRequest()->isPost()) {
            ...
            if($form->isValid()) {
                ...
                $entityManager = $this->getEntityManager();
                $user = $entityManager->createQuery("SELECT u FROM BkdUser\Entity\User u WHERE u.email = '$usernameOrEmail' OR u.username = '$usernameOrEmail'")->getResult(\Doctrine\ORM\Query::HYDRATE_OBJECT);
                $user = $user[0];
                ...
                if(isset($user)) {
                    try {
                        ...
                        $viewModel->setTemplate('bkd-user/registration/password-change-success');
                        return $viewModel;
                    } catch (\Exception $e) {
                        return $this->getServiceLocator()->get('bkduser_error_view')->createErrorView(
                                $this->getTranslatorHelper()->translate('Something went wrong when trying to send activation email! Please, try again later.'),
        ...
        if($this->getRequest()->isPost()) {
            ...
            if($form->isValid()) {
                ...
                if(UserCredentialsService::verifyHashedPassword($user, $this->params()->fromPost('password'))) {
                    ...
                    $viewModel->setTemplate('bkd-user/registration/change-email-success');
                    return $viewModel;
                } else {
        ...
        if($this->getRequest()->isPost()) {
          ...
          if($form->isValid()) {
            ...
            if(UserCredentialsService::verifyHashedPassword($user, $this->params()->fromPost('password'))) {
              ...
              $viewModel = new ViewModel(array('navMenu' => $this->getOptions()->getNavMenu()));
              $viewModel->setTemplate('bkd-user/registration/change-security-question-success');
              return $viewModel;
            } else {
    ...
    public function confirmEmailAction()
    {
        ...
        try {
            $entityManager = $this->getEntityManager();
            if($token !== '' && $user = $entityManager->getRepository('BkdUser\Entity\User')->findOneBy(array('registrationToken' => $token))) {
                $user->setRegistrationToken(md5(uniqid(mt_rand(), true)));
                $user->setState($entityManager->find('BkdUser\Entity\State', 2));
                ...
                $viewModel->setTemplate('bkd-user/registration/confirm-email-success');
                return $viewModel;                        
            } else {
    ...
    private function getOptions()
    {
        if(null === $this->options) {
            $this->options = $this->getServiceLocator()->get('bkduser_module_options');
        }
    ...
    private function getUserFormHelper()
    {
        if(null === $this->userFormHelper) {
            $this->userFormHelper = $this->getServiceLocator()->get('bkduser_user_form');
        }
